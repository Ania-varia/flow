find_RR_dates <- function(whole_dt, param, by_sex=FALSE, min_ind=70){
  ...
  if(!is.data.table(whole_dt)){
    #making sure the input is data table
    whole_dt <-data.table(whole_dt)
  }
  if(by_sex){
    #when splitting by sex, simply run for each subset and concatenate output
  ...
  test_BM <- test_BM [order(Assay_Date)] 
  ...
  #finding indices - this is max and min because of some history, feel free to make it faster with first/last element approach
  #find start indices
  ...
  #fill start and end dates for days with enough samples available before/after (better too many than too few, hence ceiling)
  ...
  #fill start and end dates for days at the beginning/end (without enough  samples available before/after)
  ...
  misising_BM <- test_BM[Assay_Date%in%missing_dates][, .(miss_date=Assay_Date, .N), by=Assay_Date]
  setkey(misising_BM, Assay_Date)
  all_res <-rbind(test_BM_WT[,c("Assay_Date", "st_date", "end_date"), with=F], test_BM_WT[misising_BM, roll=T][,c("Assay_Date", "st_date", "end_date"), with=F])[order(Assay_Date)]
  all_res[,sex:="both"]
